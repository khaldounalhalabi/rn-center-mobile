...
export function ThemedSelect({
  ...
  const [remoteOptions, setRemoteOptions] = useState<{ label: string; value: string }[]>([...options]);
  ...
  const fetchOptions = async () => {
    ...
    try {
      ...
      setRemoteOptions(data.map((item: any) => ({ label: item.label, value: item.value })));
    } catch (error) {
    ...
    <ThemedView style={styles.container}>
      {loading ? (
        ...
        <Dropdown
            ...
            onChange={(item) => { onSelect(item.value); setIsFocus(false); }}
            ...
            renderItem={(item) => (
                <View style={[{backgroundColor} ,styles.item, item.value === selectedValue ? styles.selectedItem : {}]}>
...
const styles = StyleSheet.create({
  ...
  link: {
    ...
    selectedItem: {
        backgroundColor: '#0a7ea4', // Custom background color for selected item
        padding: 10,
